...
public class PublicServicePaymentClient {

	public static void main(String[] args) throws Exception {
		
		long t1 = System.currentTimeMillis();
		
		System.out.println("Start PublicServicePaymentClient");
		...
		Service service = kb.readService("http://localhost:8080/PublicServices/PublicServicePayment/MakePayment.owl");
	
		Process process = service.getProcess();

		// get input
		kb.read("file:test/PublicServicePaymentClientInput.owl");
		OWLClass Payment = kb
				.getClass(new URI(
						"http://localhost:8080/PublicServices/PublicServicePayment/PublicServicePaymentOntology.owl#Payment"));
		OWLIndividual aPayment = kb.getInstances(
				Payment).individualAt(0);
		...
		for(int i= 0;i<inputs.size();i++){
			...
			if(input.getLabel().equals("Payment")){
		...
		System.out.println(Utils.formatRDF(aPayment.toRDF()));
