...
public class VitalRecordsClient1 {

	public static void main(String[] args) throws Exception {
		
		long t1 = System.currentTimeMillis();
		
		System.out.println("Start VitalRecordsClient1");
		...
		Service service = kb.readService("http://localhost:8080/PublicServices/VitalRecordsOffice/GetVitalRecordsDocumentInfo.owl");
	
		Process process = service.getProcess();

		// get input
		kb.read("file:test/VitalRecordsClient1Input.owl");
		...
		for(int i= 0;i<inputs.size();i++){
			...
			if(input.getLabel().equals("VitalRecordsDocumentType")){
		...
		for(int i= 0;i<outputs.size();i++){
			...
			if(output.getLabel().equals("DocumentInfo")){
		...
		System.out.println(Utils.formatRDF(aVitalRecordsDocumentType.toRDF()));
